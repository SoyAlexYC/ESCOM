                ...
                success: (respAX) => {
                    ...
                    cursos.forEach((element, index) => {
                        if (element.Estatus == 1) {
                            ...
                            <td> ${element.IdGrupo} </td>
                            <td> ${element.IdMateria} </td>
                            <td>
                                <button class="btn btn-sm btn-primary"><i class="fa-solid fa-pencil"></i></button>
                                <button class="btn btn-sm btn-danger" onclick="eliminarCurso(${element.IDCurso})"><i class="fa-solid fa-trash-can"></i></button>
                            </td>
                        </tr>`
                        }
                    });

                    tabla_cursos.innerHTML = content;
                    resolve();
                },
                error: (err) => {
...
create.addEventListener('click', () => {
    ...
    let gru = document.createElement("select");
    gru.setAttribute("id", "gru");
    gru.setAttribute("name", "gru");
    gru.setAttribute("class", "form-select");
    col_gru.appendChild(gru);
    element.appendChild(col_gru);
    let col_mat = document.createElement("td");
    let mat = document.createElement("select");
    mat.setAttribute("id", "mat");
    mat.setAttribute("name", "mat");
    mat.setAttribute("class", "form-select");
    col_mat.appendChild(mat);
        ...
        success: (respAX) => {
            ...
            if (objAX.code == 1) {
                ...
                for (let i = 0; i < objAX.data.length; i++) {
                    ...
                    para.setAttribute("name", objAX.data[i].IDGrupo);
                    para.setAttribute("id", objAX.data[i].IDGrupo);
                    let node = document.createTextNode(objAX.data[i].NombreGru);
    ...
    $.ajax({
        ...
        success: (respAX) => {
            ...
            if (objAX.code == 1) {
                ...
                for (let i = 0; i < objAX.data.length; i++) {
                    let para = document.createElement("option");
                    para.setAttribute("name", objAX.data[i].IDMateria);
                    para.setAttribute("id", objAX.data[i].IDMateria);
                    let node = document.createTextNode(objAX.data[i].NombMateria);
                    para.appendChild(node);
                    element.appendChild(para);
                }
            }
                ...
                let para = document.createElement("option");
                para.setAttribute("name", "none");
                let node = document.createTextNode("No fue posible obtener las materias");
                para.appendChild(node);
                element.appendChild(para);
            }
        }
