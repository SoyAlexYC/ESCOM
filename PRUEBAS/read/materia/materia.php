...
<body>
  ...
  $search = $_GET["materia"];
    ...
    if ($mysqli->connect_errno) {
        ...
        $nombre = $mysqli->query("SELECT * FROM (((((profesor p inner join horario h on p.IdProfesor = h.IdProfesor) inner join espacio e on h.IDHorario = e.IdHorario) inner join clase c on e.IdClase = c.IDClase) inner join curso u on c.IdCurso = u.IDCurso) inner join materia m on u.IdMateria = m.IDMateria) WHERE m.IDMateria = '$search'"); //muestras todos y ps ya ves si seleccionas uno xdd.
